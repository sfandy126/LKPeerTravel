...
@class LKTrackDetailSubCommentCell;
@interface LKTrackDetailCommentCell ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation LKTrackDetailCommentCell
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[LKTrackDetailSubCommentCell class] forCellReuseIdentifier:kLKTrackDetailSubCommentCellIdentify];
    }
    return _tableView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    LKTrackDetailSubCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:kLKTrackDetailSubCommentCellIdentify forIndexPath:indexPath];
    LKTrackCommentModel *item = [self.subComments objectAt:indexPath.section];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    LKTrackCommentModel *item = [self.subComments objectAt:indexPath.section];
    return [LKTrackDetailSubCommentCell getSubCellHeight:item];
}
...
- (void)configData:(LKTrackCommentModel *)data indexPath:(NSIndexPath *)indexPath{
    ...
    self.contentLab.text = data.commentContent;
    self.contentLab.height = [[self class] getCommentContentHeight:data.commentContent];
...
+ (CGFloat )getSubCommentTableHeight:(NSArray *)subConments{
    ...
    if (subConments.count>0) {
        ...
        for (LKTrackCommentModel *subItem in subConments) {
            subContentH += [[self class] getSubCommentContentHeight:subItem.commentContent];
        }
...
+ (CGFloat)getCellHeight:(LKTrackCommentModel *)data indexPath:(NSIndexPath *)indexPath{
    ...
    //第一级回复内容高度
    CGFloat contentH = [[self class] getCommentContentHeight:data.commentContent];
    cellHeight +=contentH;
...
@implementation LKTrackDetailSubCommentCell
...
- (void)configData:(LKTrackCommentModel *)subItem{
    self.contentLab.text = subItem.commentContent;
    self.contentLab.width = (kScreenWidth -(20+40+10) - 20-20);
    CGSize subContentSize = [LKUtils sizeFit:[NSString stringValue:subItem.commentContent] withUIFont:kFont(14) withFitWidth:self.contentLab.width withFitHeight:1000];
...
+ (CGFloat )getSubCellHeight:(LKTrackCommentModel *)subItem{
    if (subItem) {
        CGSize subContentSize = [LKUtils sizeFit:[NSString stringValue:subItem.commentContent] withUIFont:kFont(14) withFitWidth:(kScreenWidth -(20+40+10) - 20-20) withFitHeight:1000];

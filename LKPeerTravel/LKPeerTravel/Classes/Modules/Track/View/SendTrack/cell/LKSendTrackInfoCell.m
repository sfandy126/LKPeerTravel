...
@interface LKSendTrackInfoView : UIView
...
@implementation LKSendTrackInfoView
...
@interface LKSendTrackInfoCell ()
@property (nonatomic,strong) UIView *bgView;
@property (nonatomic,strong) NSArray *infoViews;
@property (nonatomic,strong) UIImageView *arrowIcon;
@end

@implementation LKSendTrackInfoCell
...
- (UIImageView *)arrowIcon{
    if (!_arrowIcon) {
        _arrowIcon = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 24, 24)];
        _arrowIcon.image = [UIImage imageNamed:@"btn_foot_into_none"];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        for (NSString *title in titles) {
            LKSendTrackInfoView *view = [[LKSendTrackInfoView alloc] initWithFrame:CGRectMake(0, 0, self.bgView.width, ceil(kBFont(12).lineHeight))];
...
- (void)configData:(LKSendTrackInfoModel *)data{
    ...
    NSInteger index = 0;
    for (LKSendTrackInfoView *view in self.infoViews) {

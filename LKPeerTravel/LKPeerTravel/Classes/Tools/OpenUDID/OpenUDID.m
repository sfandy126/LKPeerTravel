...
//#define OpenUDIDLog(fmt, ...) NSLog((@"%s [Line %zd] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define OpenUDIDLog(fmt, ...) NSLog((@"[Line %zd] " fmt), __LINE__, ##__VA_ARGS__);
...
#if TARGET_OS_IPHONE	
//    if([UIDevice instancesRespondToSelector:@selector(unique_Identifier)]){
//        _openUDID = [[UIDevice currentDevice] unique_Identifier];
//    }
#endif
    ...
    // !!!!! IMPORTANT README !!!!!
    //
    // August 2012: iOS 6 introduces new APIs that help us deal with the now deprecated [UIDevice unique_Identifier]
...
# error                                                         ^ read comments above, fix accordingly, and remove this #error line
    ...
    if (_openUDID==nil) {
        ...
        CC_MD5( cStr, (CC_LONG)(CC_LONG)strlen(cStr), result );
        CFRelease(uuid);
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%zd",kOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
